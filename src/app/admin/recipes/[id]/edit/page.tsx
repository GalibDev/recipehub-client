...
export default function AdminEditRecipePage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['admin-recipe-edit', params.id],
    queryFn: async () => {
  ...
  return (
    <>
      ...
      <p className="mt-2 text-base-content/55">Admins can update any recipe details from here.</p>
      <RecipeForm recipe={data} redirectTo="/admin/recipes" />
    </>
  );
}
